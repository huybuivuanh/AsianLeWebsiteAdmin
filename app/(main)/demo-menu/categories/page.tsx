...
import { AddDemoCategoryModal } from "@/components/demo-menu/categories/AddDemoCategoryModal";
import { EditDemoCategoryModal } from "@/components/demo-menu/categories/EditDemoCategoryModal";
...
export default function DemoCategoriesPage() {
  const {
    ...
    updateDemoCategory,
    deleteDemoCategory,
    updateDemoCategoryItemIds,
    reorderDemoCategories,
  } = useDemoCategoriesStore();
  const { items, updateDemoMenuItemField } = useDemoMenuItemsStore();
  const [modalOpen, setModalOpen] = useState(false);
  const [editingCategory, setEditingCategory] = useState<DemoCategory | null>(
    null,
  );
  const [addItemsCategory, setAddItemsCategory] = useState<DemoCategory | null>(
    null,
  );
  ...
  async function handleDelete(category: DemoCategory) {
    ...
    try {
      await deleteDemoCategory(category.id);
      for (const item of items) {
        if (item.categoryIds?.includes(category.id)) {
          const nextCategoryIds = (item.categoryIds ?? []).filter(
            (cid) => cid !== category.id,
          );
          await updateDemoMenuItemField(item.id, { categoryIds: nextCategoryIds });
        }
      }
    } finally {
      setDeletingId(null);
  ...
  async function handleRemoveItem(category: DemoCategory, itemId: string) {
    const next = (category.itemIds ?? []).filter((id) => id !== itemId);
    await updateDemoCategoryItemIds(category.id, next);
    const item = items.find((m) => m.id === itemId);
    if (item) {
      const nextCategoryIds = (item.categoryIds ?? []).filter(
        (cid) => cid !== category.id,
      );
      await updateDemoMenuItemField(itemId, { categoryIds: nextCategoryIds });
    }
  }
  ...
  async function handleSaveCategoryItems(
    ...
    await updateDemoCategoryItemIds(categoryId, itemIds);
    ...
    <div className="min-w-0">
      ...
      {loading ? (
        ...
        <p className="text-foreground/70 text-sm sm:text-base">
          No demo categories yet. Add one to get started.
        </p>
      ...
      <AddDemoCategoryModal
        open={modalOpen}
        onClose={() => setModalOpen(false)}
        onAdd={addDemoCategory}
      />

      <EditDemoCategoryModal
        open={editingCategory != null}
        category={editingCategory}
        onClose={() => setEditingCategory(null)}
        onSave={updateDemoCategory}
      />

      <AddDemoCategoryItemsModal
        open={addItemsCategory != null}
        category={addItemsCategory}
        onClose={() => setAddItemsCategory(null)}
        onSave={handleSaveCategoryItems}
      />
      <SortDemoCategoriesModal
        open={sortModalOpen}
        categories={categories}
        onClose={() => setSortModalOpen(false)}
        onSave={reorderDemoCategories}
      />
    </div>
  );
}
