...
type AddDemoCategoryModalProps = {
  open: boolean;
  onClose: () => void;
  onAdd: (name: string, description: string) => Promise<void>;
};

export function AddDemoCategoryModal({
  open,
  onClose,
  onAdd,
}: AddDemoCategoryModalProps) {
  ...
  return (
    <div
      ...
      aria-modal="true"
      role="dialog"
      aria-labelledby="add-demo-category-title"
    >
      <div
      ...
      <div className="relative w-full max-w-md rounded-xl bg-background border border-foreground/10 shadow-lg p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label
              htmlFor="demo-category-description"
              className="block text-sm font-medium text-foreground mb-1"
            >
              Description
            </label>
            <textarea
              id="demo-category-description"
              value={description}
