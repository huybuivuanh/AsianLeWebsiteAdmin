...
type AddUpdatesItemModalProps = {
  open: boolean;
  onClose: () => void;
  onAdd: (name: string, imageFile: File) => Promise<void>;
};

export function AddUpdatesItemModal({
  open,
  onClose,
  onAdd,
}: AddUpdatesItemModalProps) {
  ...
  return (
    <div
      ...
      aria-modal="true"
      role="dialog"
      aria-labelledby="add-updates-item-title"
    >
      <div
      ...
      <div className="relative w-full max-w-md rounded-xl bg-background border border-foreground/10 shadow-lg p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label
              htmlFor="updates-display-name"
              className="block text-sm font-medium text-foreground mb-1"
            >
              Display Name <span className="text-red-500">*</span>
            </label>
            <input
              id="updates-display-name"
              type="text"
              value={displayName}
